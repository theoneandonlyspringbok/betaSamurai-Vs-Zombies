...
public class PachinkoItemDisplayDialog : IDialog
{
	...
	public PachinkoItemDisplayDialog(StoreData.Item itemDesc)
	{
		mItemDesc = itemDesc;
		mLayout = new SUILayout("Layouts/PachinkoShopDetails");
		RenderItem();
		...
		((SUIButton)mLayout["exit"]).onButtonPressed = Exit;
		((SUILabel)mLayout["exitText"]).text = Singleton<Localizer>.instance.Get("exit");
	...
	private void RenderItem()
	{
		...
		{
			mSpoilsDisplay = new SpoilsDisplay(mLayout, mItemDesc.contentList, "default18", new Vector2(SUIScreen.width / 2f, SUIScreen.height / 2f - 20f), 2f, 5, 144f, string.Empty);
		}
	}
	...
	private void Exit()
	{
		...
		WeakGlobalSceneBehavior<PachinkoMachineImpl>.instance.RequestQuitOnNextUpdate();
	}

	private void SetupPurchaseButton(SUIButton btn, Cost cost)
	{
		btn.position = new Vector2(346f, btn.position.y);
