...
public class InGameStoreDialog
{
	public static IDialog Create(string itemID, bool applyCostMultiplier)
	{
		...
		{
			StoreItemDisplayDialog storeItemDisplayDialog = new StoreItemDisplayDialog(storeItemDesc);
			storeItemDisplayDialog.priority = 1000f;
			storeItemDisplayDialog.onItemPurchased = delegate
			{
				Singleton<Analytics>.instance.LogEvent("InGamePurchase", itemID);
			};
			return storeItemDisplayDialog;
		}
